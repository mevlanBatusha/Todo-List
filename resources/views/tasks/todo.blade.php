<ul class="todo" id="todo">
	@foreach($untasks as $task)
		<li>{{ $task->name }} 

			<div class="buttons">
				<form action="{{ url('task/' .$task->id ) }}" method="POST" >
					<button class="remove" id="deleteData">
						...
						<input type="hidden" id="delId" value="{{ $task->id }}">	
						{{ method_field('DELETE') }}
						{{ csrf_field() }}
					</button>
				</form>
				<form action="{{ url('task/confirm/' .$task->id ) }}" method="POST" >
					...
					{{ csrf_field() }}	
					<input type="hidden" id="confId" value="{{ $task->id }}">	
				{{-- </form> --}}
			</div>
		</li>
	@endforeach
	...
	@foreach($tasks as $task)
			<li>
				...
				<div class="buttons">
					...
					<form action="{{ url('task/unconfirm/' .$task->id ) }}" method="POST"  >
						<button class="complete" id="uncomplete">
							<svg version="1.1"  xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px" viewBox="0 0 22 22" style="enable-background:new 0 0 22 22;" xml:space="preserve"><rect y="0" class="nofill" width="22" height="22"/> <g> <path class="fill" d="M9.7,14.4L9.7,14.4c-0.2,0-0.4-0.1-0.5-0.2l-2.7-2.7c-0.3-0.3-0.3-0.8,0-1.1s0.8-0.3,1.1,0l2.1,2.1l4.8-4.8 c0.3-0.3,0.8-0.3,1.1,0s0.3,0.8,0,1.1l-5.3,5.3C10.1,14.3,9.9,14.4,9.7,14.4z"/> </g> </svg>
						</button>
						{{ csrf_field() }}	
						<input type="hidden" id="unconfId" value="{{ $task->id }}">		
					</form>
				</div>
			</li>
	@endforeach
